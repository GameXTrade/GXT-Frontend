...
import { Card, Typography, Tooltip, IconButton, Button, Chip, CardHeader, CardBody, CardFooter } from "@material-tailwind/react";
...
const TABLE_HEAD = ["Preview","Name", "Status", "Downloads", "Price", "Actions"];
...
export function ActiveTableWithStripedRows() {
  ...
  const paginatedRows = TABLE_ROWS.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);

  return (
    <Card className="flex flex-col shadow-none border bg-white" style={{ height: "calc(100dvh - 83px)" }}>
      <CardHeader floated={false} className="rounded-none shadow-none m-0 h-20 bg-transparent p-2">
        <div className="flex h-full  items-center justify-between px-3">
            ...
            <div>
              <Button className="w-13 h-13" onClick={toggleModal}>Upload Asset</Button>
            </div>
        </div>
      ...
      <CardBody className="flex-1 p-0 t-0">
        <table className="w-full min-w-max table-auto text-left">
          ...
          <tbody>
            {paginatedRows.map(({item_id, imagelink,  name, activated, download_count, price }, index) => (
              <tr key={name} className="border-b border-indigo-50 hover:bg-blue-gray-50/50">
                <dt className="p-4">
